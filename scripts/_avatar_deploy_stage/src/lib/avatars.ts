/** Preset catalog — SVGs under `public/avatars/presets/`. */
...
export function presetAvatarUrl(id: AvatarPresetId | string): string {
  return `${AVATAR_PRESET_PREFIX}${id}.svg`;
}
...
export function isPresetAvatarUrl(url: string): boolean {
  if (!url.startsWith(AVATAR_PRESET_PREFIX) || !url.endsWith(".svg")) {
    return false;
  }
  const id = url.slice(AVATAR_PRESET_PREFIX.length, -".svg".length);
...
export function isUploadedAvatarUrl(url: string): boolean {
  ...
  return /^[a-zA-Z0-9._-]+\.(jpe?g|png|webp)$/i.test(name);
}
