...
const SHADOWS = { sm: [5, 12, 0.3], md: [10, 24, 0.4], lg: [16, 40, 0.52] } as const;
...
export type CurvedInputProps = {
  ...
  placeholder?: string;
  buttonText?: string;
  /** Left of primary curved button (e.g. 浏览服务). */
  ...
  iconColor?: string;
  shadowSize?: keyof typeof SHADOWS | "none";
  shadowColor?: string;
  showButton?: boolean;
  showIcon?: boolean;
  icon?: ReactNode;
...
export default function CurvedInput({
  ...
  const secondaryPathId = `ci-sec-${uid}`;
  ...
  const secMeasureRef = useRef<SVGTextElement>(null);
  ...
  const [secTextW, setSecTextW] = useState(0);
  ...
  const secBg = secondaryButtonColor ?? "rgba(255,255,255,0.12)";
  const secFg = secondaryButtonTextColor ?? fgColor;
  const shColor = shadowColor ?? palette.shadowColor;
  const hasSecondary = showSecondaryButton && Boolean(secondaryButtonText);
  ...
  const layout = useMemo(() => {
    ...
    const primaryW = showButton ? Math.max(btnTextW + fontSize * 2.7, T * 1.35) : 0;
    const secondaryW = hasSecondary
      ? Math.max(secTextW + fontSize * 2.2, T * 1.15)
      : 0;
    const gap = hasSecondary && showButton ? 10 : 0;
    const btnU1 = geom.W - btnInset;
    const btnU0 = btnU1 - primaryW;
    const secU1 = showButton ? btnU0 - gap : btnU1;
    const secU0 = secU1 - secondaryW;
    const textEndU = Math.max(
      textStartU + 20,
      (hasSecondary ? secU0 : showButton ? btnU0 : geom.W) - 14
    );
  ...
  useLayoutEffect(() => {
    ...
    if (secMeasureRef.current) {
      const sw = secMeasureRef.current.getComputedTextLength();
      setSecTextW((prev) => (Math.abs(prev - sw) > 0.5 ? sw : prev));
    }
  ...
  if (geom && layout) {
    ...
    const secondaryPath = hasSecondary
      ...
      : "";
    const secondaryTextPath = hasSecondary
      ? bentLinePath(geom, layout.secU0, layout.secU1, vBase)
      : "";

    const stopBtn = (e: ReactPointerEvent | KeyboardEvent | ReactMouseEvent) => {
      e.stopPropagation();
    };
    ...
    content = (
      ...
      >
        ...
        {hasSecondary ? (
          ...
          >
            <path className="curved-input__button-bg" d={secondaryPath} fill={secBg} />
