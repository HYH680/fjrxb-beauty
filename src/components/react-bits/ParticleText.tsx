...
export interface ParticleTextProps {
  ...
  className?: string;
  style?: CSSProperties;
  /** Fires once when the mount gather animation finishes (or immediately under reduced motion). */
...
const ParticleText = ({
  ...
  const onGatherCompleteRef = useRef(onGatherComplete);
  onGatherCompleteRef.current = onGatherComplete;
  ...
  useEffect(() => {
    ...
    const notifyGatherComplete = (): void => {
      if (gatherNotified) return;
      gatherNotified = true;
      onGatherCompleteRef.current?.();
    };
