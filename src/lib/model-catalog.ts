...
export const MODEL_CATALOG: ModelProfile[] = [
  ...
  },
  {
    id: "hunyuan-hy3",
    label: "混元 Hy3",
    why: "腾讯 TokenHub 旗舰；中文对话与通用任务，作千问之外的国内备选。",
    kind: "chat",
    provider: "tokenhub",
    model: "hy3",
    scores: {
...
function profileConfig(profile: ModelProfile): LlmConfig | null {
  ...
  // FreeLLMAPI / TokenHub 只补缺；没有自己的 key 时不要靠 new-api 网关冒充可用
  if (profile.provider === "freellmapi" || profile.provider === "tokenhub") {
...
function profileConfigIsUsable(
  ...
  if (profile.provider === "tokenhub") return Boolean(config.apiKey);
...
const TASK_BY_PRODUCT: Record<string, TaskKind> = {
  ...
  "pinecone-avatars": "code",
  dicebear: "code",
  "upload-crop-image": "code",
  uploadthing: "code",
  "supabase-avatar-example": "code",
  "langchain-pro": "code",
  ...
  "brand-visual": "chinese-copy",
  "poster-layout": "chinese-copy",
  "ui-wireframe": "chinese-copy",
  "packaging-design": "chinese-copy",
...
export function catalogRecommendForProduct(product: Product) {
  const textTask = productTask(product, { hasImages: false });
  const visionCapable = isVisionProduct(product);

  const rankFor = (task: TaskKind) =>
    MODEL_CATALOG.filter((profile) => {
      if (task === "vision") {
  ...
  const textRanked = rankFor(textTask);
  const primary = textRanked[0];
  const fallback = textRanked[1];
  const visionPrimary = visionCapable ? rankFor("vision")[0] : null;

  return {
    task: textTask,
    taskLabel: TASK_LABEL[textTask],
    primaryLabel: primary?.label || "待分配",
    primaryWhy: primary?.why || "",
    fallbackLabel:
      visionCapable && visionPrimary
        ? `有图 ${visionPrimary.label}${fallback ? ` · 备选 ${fallback.label}` : ""}`
        : fallback?.label || "",
    autoHint: primary
      ? visionCapable && visionPrimary
        ? `推荐 ${primary.label} · 有图走${visionPrimary.label}`
        : `推荐 ${primary.label} · ${TASK_LABEL[textTask]}（省钱）`
      : `自动匹配 · ${TASK_LABEL[textTask]}`,
  };
}
